...
const createSchema = z.object({ name: z.string().min(1), type: z.string().optional(), target: z.number().positive(), saved: z.number().optional(), monthlyContribution: z.number().optional(), deadline: z.string().optional() });
...
export async function PUT(req: Request) {
  ...
  try {
    await connectDB();
    const updated = await GoalModel.findOneAndUpdate({ _id: id, userId: auth.userId }, parsed.data, { new: true }).lean();
