...
function buildDemoHistory(now = new Date()) {
  ...
  for (const month = new Date(firstMonth); month < currentMonth; month.setMonth(month.getMonth() + 1)) {
  ...
  const expenses = months.flatMap((month, index) => {
    ...
    return [
      { userId: DEMO_EMAIL, amount: rent, category: "Rent", merchant: "Landlord", paymentMethod: "Bank Transfer", date: date(1), recurring: true },
      { userId: DEMO_EMAIL, amount: 3100 + (index % 4) * 260, category: "Groceries", merchant: "BigBasket", paymentMethod: "UPI", date: date(6) },
      { userId: DEMO_EMAIL, amount: 1550 + (index % 3) * 175, category: "Utilities", merchant: "Electricity Board", paymentMethod: "UPI", date: date(11), recurring: true },
      { userId: DEMO_EMAIL, amount: 499, category: "Subscriptions", merchant: "Netflix", paymentMethod: "Card", date: date(14), recurring: true },
      { userId: DEMO_EMAIL, amount: 1250 + (index % 5) * 140, category: "Food", merchant: "Swiggy", paymentMethod: "UPI", date: date(18) },
      { userId: DEMO_EMAIL, amount: 780 + (index % 4) * 95, category: "Travel", merchant: "Uber", paymentMethod: "UPI", date: date(21) },
      { userId: DEMO_EMAIL, amount: 1700 + (index % 6) * 230, category: "Shopping", merchant: "Myntra", paymentMethod: "Card", date: date(23) },
    ];
  });

  const incomes = months.flatMap((month, index) =>
    index % 3 === 2
      ? [{ userId: DEMO_EMAIL, amount: 4500 + index * 150, type: "Freelance", source: "Design project", date: new Date(month.getFullYear(), month.getMonth(), 16) }]
      : [],
  );
...
export async function POST(request: Request) {
  ...
  await Promise.all([
    ...
    BillModel.insertMany([
      { userId: DEMO_EMAIL, name: "Rent", amount: 22000, dueDay: 1, frequency: "monthly", category: "Rent", paid: true },
      { userId: DEMO_EMAIL, name: "Electricity", amount: 1800, dueDay: 12, frequency: "monthly", category: "Utilities", paid: false },
      { userId: DEMO_EMAIL, name: "Internet", amount: 999, dueDay: 15, frequency: "monthly", category: "Utilities", paid: false },
      { userId: DEMO_EMAIL, name: "Netflix", amount: 499, dueDay: 20, frequency: "monthly", category: "Subscriptions", paid: false },
    ]),
    GoalModel.insertMany([
      { userId: DEMO_EMAIL, name: "Emergency Fund", type: "Emergency Fund", target: 300000, saved: 180000, monthlyContribution: 10000 },
      { userId: DEMO_EMAIL, name: "Goa Vacation", type: "Vacation", target: 60000, saved: 24000, monthlyContribution: 6000 },
    ]),
    InvestmentModel.insertMany([
      { userId: DEMO_EMAIL, name: "Nifty 50 Index", type: "SIP", invested: 120000, currentValue: 148000, monthly: 5000 },
      { userId: DEMO_EMAIL, name: "Parag Parikh Flexi", type: "Mutual Funds", invested: 90000, currentValue: 112500, monthly: 3000 },
    ]),
    BankAccountModel.insertMany([
      { userId: DEMO_EMAIL, bankName: "HDFC Salary", accountType: "Salary", balance: 42840, defaultFor: ["everyday", "obligations"] },
      { userId: DEMO_EMAIL, bankName: "SBI Savings", accountType: "Savings", balance: 180000, defaultFor: ["savings"] },
    ]),
    ...
    NotificationModel.insertMany([
      { userId: DEMO_EMAIL, title: "Bill due soon", body: "Electricity bill of ₹1,800 is due on the 12th.", type: "bill" },
      { userId: DEMO_EMAIL, title: "Goal on track", body: "Your emergency fund is now 60% complete.", type: "goal" },
    ]),
  ]);
