...
export async function POST(req: Request) {
  ...
  const ipLimit = await consumeRateLimit({ scope: "reset-otp-ip", identifier: getClientIp(req), limit: 10, windowMs: 15 * 60 * 1000 });
  const emailLimit = await consumeRateLimit({ scope: "reset-otp-email", identifier: parsed.data.email, limit: 3, windowMs: 15 * 60 * 1000 });
