# SalaryFlow Finance Calculation Contract

This document defines the source of truth for SalaryFlow calculations and UI bindings. Code changes that affect money should preserve these rules and update the focused tests in `src/lib/finance.test.ts`.
...
The `balanceApplied` marker records that SalaryFlow performed the deduction. It allows edit, delete, and recycle-bin restore operations to reverse or reapply the amount exactly once.
